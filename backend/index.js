...
console.log(investorRequests);
        // Combine both request arrays (can handle either or both being empty)
        ...
        // Transform the data with null checks for populated fields to handle empty arrays
        const transformedRequests = combinedRequests.map(request => {
            const startupData = request.startup_id || {}; // Safely handle missing startup data

            return {
                _id: request._id,
                startup_id: startupData._id || null, // Use null if startup_id is missing
                startup_name: startupData.name || 'Unknown Startup', // Fallback if title missing
                message: request.message || '',
                funding_amount: request.funding_amount || request.proposed_investment || 0,
                equity_offered: request.equity_offered || request.equity_asked || 0,
                status: request.status,
                created_at: request.created_at,
                updated_at: request.updated_at,
                companyValuation: startupData.companyValuation || 0,
                companyFounded: startupData.companyFounded || 'N/A',
                shortDescription: startupData.shortDescription || 'No description available',
                location: startupData.location || 'Unknown',
                email: startupData.email || 'Not provided',
                returnOnInvestment: startupData.returnOnInvestment || 0,
            };
        });
...
app.get('/api/messages', async (req, res) => {
  ...
  app.get('/api/investors/:startupId', async (req, res) => {
    ...
    try {
        ...
        console.log("p"+investorRequests);
        const startupRequests = await StartupRequest.find({
            startup_id: startupId,
            status: 'accepted'
        }).populate({
            path: 'investor_id',
            model: 'Profile',
            select: 'userId title email contactNumber companyFounded shortDescription totalInvestments totalFundsInvested bio gender birthdate location returnOnInvestment profilePhoto'
        });
    ...
    console.log(combinedRequests);
        ...
        const transformedInvestors = combinedRequests.map(request => {
            const profile = request; // Access populated profile data
            
            return {
                _id: request._id,
                investor_id: profile?.investor_id._id,
                investor_name: profile?.investor_id.name|| 'Unknown',
                message: request.message,
                ...
                totalFundsInvested: profile?.totalFundsInvested || 0,
                bio: profile?.message || 'No bio available',
                location: profile?.location || 'Unknown',
                returnOnInvestment: profile?.returnOnInvestment || 'N/A',
                profilePhoto: profile?.profilePhoto || '',
            };
        });
     console.log(transformedInvestors);
        return res.status(200).json({
            success: true,
